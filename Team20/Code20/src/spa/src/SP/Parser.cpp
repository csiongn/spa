...
void Parser::throwParseError(const std::string &message) {
  std::cerr << "Encountered error while parsing token " << current + 1 << ": '" << tokens[current].value << "'"
			<< std::endl;
...
std::shared_ptr<ExprNode> Parser::relExpr() {
  ...
  if (match({SP::TokenType::GREATER_THAN, SP::TokenType::GREATER_THAN_EQUAL, SP::TokenType::LESS_THAN,
			 SP::TokenType::LESS_THAN_EQUAL, SP::TokenType::DOUBLE_EQUAL, SP::TokenType::NOT_EQUAL})) {
