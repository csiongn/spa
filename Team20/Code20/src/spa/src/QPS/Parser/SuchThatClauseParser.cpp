...
PQL::Clause SuchThatClauseParser::parse(std::vector<std::shared_ptr<QueryToken>> &suchThatClauseTokens) {
    ...
    std::vector<std::shared_ptr<QueryToken>> cleanedSuchThatTokens = QueryEvaluator::ParseUtils::removeBracketsAndCommas(suchThatClauseTokens);
    std::vector<std::shared_ptr<QueryToken>> suchThatArgsTokens = QueryEvaluator::ParseUtils::splitTokens(cleanedSuchThatTokens, 3, cleanedSuchThatTokens.size());

    if (suchThatClauseType == SimpleProgram::DesignAbstraction::MODIFIESS || suchThatClauseType == SimpleProgram::DesignAbstraction::USESS) {
        ...
        args.push_back(lArg);
        args.push_back(rArg);
    } else if (suchThatClauseType == SimpleProgram::DesignAbstraction::MODIFIESP || suchThatClauseType == SimpleProgram::DesignAbstraction::USESP) {
        auto lArgToken = suchThatArgsTokens[0];
        auto rArgToken = suchThatArgsTokens[1];
    ...
    }   else if (suchThatClauseType == SimpleProgram::DesignAbstraction::PARENT || suchThatClauseType == SimpleProgram::DesignAbstraction::PARENTT ||
        suchThatClauseType == SimpleProgram::DesignAbstraction::FOLLOWS || suchThatClauseType == SimpleProgram::DesignAbstraction::FOLLOWST) {
        auto lArgToken = suchThatArgsTokens[0];
        auto rArgToken = suchThatArgsTokens[1];
