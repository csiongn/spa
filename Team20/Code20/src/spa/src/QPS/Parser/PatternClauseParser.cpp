...
PQL::Clause PatternClauseParser::parse(std::vector<std::shared_ptr<QueryToken>> &patternClauseTokens) {
  ...
  if (patternClauseType == SimpleProgram::DesignAbstraction::PATTERN_ASSIGN) {
	...
	if (patternArgsTokens.size() == 4) {
	  ...
	  std::vector<std::shared_ptr<QueryToken>>
		  rargTokens = QueryEvaluator::ParseUtils::splitTokens(patternArgsTokens, 1, patternArgsTokens.size());
	...
	if (isPartialExpression) {
	  patternArgs.push_back(QueryEvaluator::ParseUtils::createSynonym(SimpleProgram::DesignEntity::PARTIAL_EXPR,
																	  partialExpressionId));
	} else if (rargToken->getType() == QPS::TokenType::WILDCARD) {
	  patternArgs.push_back(QueryEvaluator::ParseUtils::createSynonym(SimpleProgram::DesignEntity::WILDCARD,
																	  rargToken));
	} else {
