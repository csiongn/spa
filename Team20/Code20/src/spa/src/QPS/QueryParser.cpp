...
std::tuple<bool,
		   SimpleProgram::DesignEntity> QueryParser::verifyDeclarationExists(const std::shared_ptr<QueryToken> &token) {
  ...
  for (const auto &syn : initialDeclarations) {
	if (syn.identity == tokenId) {
	  ...
	  bool usedDeclarationExists =
		  std::find(std::begin(usedDeclarations), std::end(usedDeclarations), syn) != std::end(usedDeclarations);
...
bool QueryParser::isExpSpec(const std::vector<std::shared_ptr<QueryToken>> &expSpecTokens) {
  if (expSpecTokens.empty()) {
	...
	return expSpecTokens[0]->getType() == QPS::TokenType::WILDCARD
		|| expSpecTokens[0]->getType() == QPS::TokenType::CONSTANT_STRING && isFactor(expSpecTokens[0]);
  } else if (expSpecTokens.size() == 3) {
	return expSpecTokens[0]->getType() == QPS::TokenType::WILDCARD
		&& expSpecTokens[2]->getType() == QPS::TokenType::WILDCARD && isFactor(expSpecTokens[1]);
  } else {
...
bool QueryParser::isValidRelationship(int start, SimpleProgram::DesignAbstraction relationship) {
  ...
  if (isFollowsOrParent && hasValidFirstArgument && firstArgToken->getType() == QPS::TokenType::NAME
	  && !isStmtSubtype(firstArgToken)) {
	throw QuerySemanticError(
		"Semantic Error: first argument must be a statement synonym, or a subtype of a statement synonym (read, print, assign, if, while, call)");
  }
  ...
  if (isFollowsOrParent) {
	...
	if (hasValidSecondArgument && secondArgToken->getType() == QPS::TokenType::NAME && !isStmtSubtype(secondArgToken)) {
	  throw QuerySemanticError(
		  "Semantic Error: second argument must be a statement synonym, or a subtype of a statement synonym (read, print, assign, if, while, call)");
	}
  }

  if (!isFollowsOrParent) {
	hasValidSecondArgument = isEntRef(secondArgToken);
	if (isModifies && hasValidSecondArgument && secondArgToken->getType() != QPS::TokenType::CONSTANT_STRING
		&& isName(secondArgToken->getValue())
		&& getEntityTypeFromSynonym(secondArgToken) != SimpleProgram::DesignEntity::VARIABLE) {
...
bool QueryParser::isValidPatternSynonym(std::shared_ptr<QueryToken> &token) {
  ...
  for (const auto &declaration : initialDeclarations) {
	...
	if (isAllowedPatternSynonyms) {
	  ...
	  bool usedDeclarationExists = std::find(std::begin(usedDeclarations), std::end(usedDeclarations), declaration)
		  != std::end(usedDeclarations);
...
bool QueryParser::isValidPattern(int start, int end) {
  ...
  if (hasValidFirstArgument && tokens[start - 1]->getType() == QPS::TokenType::NAME) {
	...
	if (std::get<1>(isValidSynonym) != SimpleProgram::DesignEntity::VARIABLE) {
	  if (patternSynonymEntityType == SimpleProgram::DesignEntity::IF
		  || patternSynonymEntityType == SimpleProgram::DesignEntity::WHILE) {
...
std::tuple<bool, SimpleProgram::DesignAbstraction, std::vector<PQL::Synonym>> QueryParser::getRelationshipAttributes() {
  ...
  if (tokenValue == "Follows" || tokenValue == "Parent") {
	...
	if (nextPos < tokens.size()) {
	  ...
	  abstraction = tokenValue == "Follows" ? SimpleProgram::DesignAbstraction::FOLLOWS
	...
	if (nextPos < tokens.size()) {
	  ...
	  abstraction = tokenValue == "Follows*" ? SimpleProgram::DesignAbstraction::FOLLOWST
	...
	std::vector<PQL::Synonym> args;
	auto abstraction = tokenValue == "Modifies" ? SimpleProgram::DesignAbstraction::MODIFIESS
...
std::vector<PQL::Clause> QueryParser::parseClauses() {
  ...
  while (pos < tokens.size()) {
	...
	if (tokenValue == "pattern") {
	  ...
	  std::tuple<bool, SimpleProgram::DesignAbstraction, std::vector<PQL::Synonym>>
		  verification = getRelationshipAttributes();
